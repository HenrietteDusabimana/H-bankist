...
const calcDisplaySummary = function(movements) {
  const incomes = movements.filter(mov => mov > 0).reduce((accumulator, mov) => accumulator + mov, 0);
  labelSumIn.textContent = `${incomes}€`;

  const out = movements.filter(mov => mov < 0).reduce((accumulator, mov) => accumulator + mov, 0);
...
// PIPELINE
const totalDepositsUSD = movements.filter(mov => mov > 0).map(mov => mov * eurToUsd).reduce((accumulator, mov) => accumulator + mov, 0);
